...
namespace FruitHAP.SensorProtocols
{
	...
	public class ACProtocol : IACProtocol
	{
		...
		private readonly byte[] ProtocolHeader = new byte[]{0x0B, 0x11};
		private const byte PacketIndicator = 0x00;
		...
		private void CheckProtocolIndicator(byte[] rawData)
		{
			...
			{
				throw new ProtocolException(string.Format("Incorrect protocol indicator. This is not an AC packet. Protocol indicator is 0x{0:X} 0x{1:X}", rawData[0], rawData[1]));
			}
		}
		...
		private void CheckLength(byte[] rawData)
		{
			if (rawData.Count() != 12)
			{
				throw new ProtocolException(string.Format("Incorrect packet length. This is not an AC packet. Length={0}", rawData.Count()));
			}
		}
